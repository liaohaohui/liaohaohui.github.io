...
# Q2 (a)
#https://liaohaohui.github.io/UECM3993/Room.csv
Room = read.csv("Room.csv")
# dim(Room)   # 144 rows, 5 columns
#head(Room)
temp = Room[,2:5]    # We just want the temperatures
apply(temp, 2, mean) # Temperatures in Faranheit??? Celcius??? > 290 (for cooking)
apply(temp, 2, sd)   # Variations OK, no need to scale
...
#https://liaohaohui.github.io/UECM3993/T5_6_PILOT.DAT
pilots = read.table('T5_6_PILOT.DAT', col.names=c('Group', 
  'Intelligence', 'Form Relations', 'Dynamometer', 'Dotting', 
  'Sensory Motor Coordination', 'Perservation'))
pilots$Group = factor(ifelse(pilots$Group == 1, 'Apprentice', 'Pilot'))
#
# We try to calculate the eigenvalues and eigenvectors of the covariance
# matrix of the data `pilots' to compare the result to `prcomp'
#
S = cov(pilots[,2:7])
s.eigen = eigen(S)
PVE = s.eigen$values/sum(s.eigen$values)
...
s.eigen$vectors
#
# vs the standard `prcomp' PCA
#
pilots.pca = prcomp(pilots[,2:7])
summary(pilots.pca)

# To recreate the graph generated by ggfortify::autoplot(), scale the data 
# using the standard deviations of the principal components multiplied by 
# the square root of the number of observations. The principal components 
# are then computed for each observation vector. Note the first eigenvector 
# is multiplied by a scaling factor of −1 so the signs what was reported 
...
#plot(d.f[,1:2], col=c(rep(1,20),rep(2,20)), pch=15+c(rep(1,20),rep(2,20)), cex=2)
...
ls()   # -> e, tab, tissue
dim(e) # class(e) -> matrix
x = t(e)   # Similar to NCI60 genetic data
pc = prcomp(x)
# Can't use biplot directly because it does not allow me to label
# the points nicely.
plot(pc$x[,1], pc$x[,2], col=as.integer(factor(tab$Tissue)), 
  pch=15, cex=2, main = "PCA", xlab = "PC1", ylab = "PC2")
text(pc$x[,1], pc$x[,2]+2, tab$Tissue, cex=0.8)
...
#https://liaohaohui.github.io/UECM3993/exprs_GSE5859.csv
gse5859 = read.csv("exprs_GSE5859.csv",row.names=1,header=TRUE)
#https://liaohaohui.github.io/UECM3993/sampleinfo_GSE5859.csv
sampleinfo = read.csv("sampleinfo_GSE5859.csv")
idx = data.frame(
...
plot(pc$sdev[1:50]/sum(pc$sdev),type="b",ylab="% singular values")
groups = factor(sampleinfo$ethnicity)
kmcls = kmeans(pc$x[,1:2], length(levels(groups)), nstart=20)  # Practical 7
